...
/* ---------------------------------------------------------
   Card builder — big, rich, clickable
--------------------------------------------------------- */
function buildOttCard(o, index) {
  ...
  return `
    <article class="ott-page-card" data-index="${index}">
      ...
      <div class="ott-page-body">
        ...
        <button type="button" class="btn btn-ghost ott-read-more" data-index="${index}">More info →</button>
      </div>
    </article>`;
}
  ...
  ottModalCard.innerHTML = `
    ...
    <div class="modal-scroll-inner">
      <div class="modal-poster">
        ${o.poster ? `<img src="${escapeHtml(o.poster)}" alt="${escapeHtml(o.title)}" loading="lazy" onerror="this.style.opacity=0" />` : `<div class="ott-no-poster" style="height:100%;">🎬</div>`}
      </div>
      <div class="modal-content">
        ...
        ${releaseBadge(o.releaseDate) ? `<div style="margin:12px 0;">${releaseBadge(o.releaseDate).replace('position:absolute;', '')}</div>` : ""}
...
function closeOttModal() { ottModal.classList.remove("active"); }
...
/* ---------------------------------------------------------
   Auto-detection (TMDB) — only movies 30+ days past release,
   only flatrate streaming, skip "Others" platform
--------------------------------------------------------- */
  ...
  return getUpcoming()
    .then((movies) => {
      const ottCandidates = movies.filter((m) => {
        ...
        const daysSinceRelease = (Date.now() - new Date(m.release_date)) / (1000 * 60 * 60 * 24);
